...
class MaskedReconstructionLoss(torch.nn.Module):
    ...
    def forward(self, input: Tensor, target: pyg.data.Data, mask, *args, **kwargs) -> Tensor:
        ...
        input_logits = input[target_edges[0], target_edges[1]]
        ...
        #   is it safe?
        loss = F.binary_cross_entropy_with_logits(input_logits, target_labels).mean()
        return loss
